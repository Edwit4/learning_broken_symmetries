...
cmds = [['python', f'{experiment_dir}/train_test.py', str(kf), str(t_size), 
    str(l_size), str(lr), str(b_size), v, str(p)] 
    for kf,t_size,l_size,lr,b_size,v,p in parameterizations]

dir_names = [f'{experiment_dir}/versions/{v}_trainsize_{t_size}_layersize_{l_size}_learningrate_{lr}_batchsize_{b_size}_fold_{kf}'
             for kf,t_size,l_size,lr,b_size,v,p in parameterizations]
