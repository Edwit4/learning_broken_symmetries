...
if __name__ == '__main__':
    ...
    data_dir = 'src/data'
    version_name = f'{version}_trainsize_{train_size}_layersize_{layer_size}_learningrate_{learning_rate}_batchsize_{batch_size}_crit1_{crit_scale}_crit2_{crit_2_scale}_fold_{fold_idx}'
    ...
    train_idx, valid_idx = kfold_split(train_valid_idx, fold_idx=fold_idx, n_splits=n_splits, 
                                       shuffle=True, rng=rng)
    print('Train count: ', len(train_idx), 'Valid count: ', len(valid_idx),
          'Test count: ', len(test_idx))
    ...
    train_dataset = toy_fcn_aug_set_pix_dataset(data_dir, indices=train_idx,
                                                scaler=scaler,
                                                pixelization=pixelization)
    val_dataset = toy_fcn_aug_set_pix_dataset(data_dir, indices=valid_idx,
                                              scaler=scaler,
                                              pixelization=pixelization)
    ...
    test_dataset = toy_fcn_dataset(data_dir, rotation=0, indices=test_idx,
                                   scaler=scaler, pixelization=pixelization)
