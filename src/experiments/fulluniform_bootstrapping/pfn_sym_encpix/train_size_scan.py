...
dir_names = [f'{experiment_dir}/versions/{version_name}_trainsize_{t_size}_layersize_{layer_size}_learningrate_{learning_rate}_batchsize_{batch_size}_crit1_{crit_scale}_crit2_{crit_2_scale}_bootstrap_{bs}'
             for bs, t_size, port in parameterizations]

def worker(cmd, name):
    if not os.path.exists(f'{name}/csv_logs/test_roc.csv'):
...
with ProcessPoolExecutor(max_workers=num_parallel) as executor:
    for i, (name, cmd) in enumerate(zip(dir_names, cmds)):
        time.sleep(2)
