...
import numpy as np
from src.utilities.models import FCN
from src.utilities.datasets import toy_fcn_dataset
...
if __name__ == '__main__':
    ...
    parser.add_argument('num_epochs', metavar='e', type=int, default=1,
    ...
    data_dir = 'src/data'
    version_name = f'{version}_trainsize_{train_size}_layersize_{layer_size}_learningrate_{learning_rate}_batchsize_{batch_size}_bootstrap_{strap_idx}'
    ...
    model = FCN(layer_size=layer_size, pixelization=pixelization)
    train_dataset = toy_fcn_dataset(data_dir, rotation=0, indices=train_idx, scaler=scaler,
                                    pixelization=pixelization)
    ...
    test_model = FCN(layer_size=layer_size, pixelization=pixelization)
