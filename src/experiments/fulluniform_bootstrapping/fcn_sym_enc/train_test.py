...
import numpy as np
from src.utilities.models import FCN_aug
from src.utilities.datasets import toy_fcn_dataset, toy_fcn_aug_set_dataset
from src.utilities.torch_utils import trainer, test
from src.utilities.preprocessing import make_toy_fcn_standardscaler
from src.utilities.data_utils import split_data, bootstrap
...
if __name__ == '__main__':
    ...
    parser.add_argument('strap_idx', metavar='f', type=int, nargs=1,
    ...
    parser.add_argument('batch_size', metavar='b', type=int, nargs=1,
        help='Batch size')
    parser.add_argument('crit_scale', metavar='a', type=float, nargs=1,
        help='Scale of first criterion')
    parser.add_argument('crit_2_scale', metavar='b', type=float, nargs=1,
        help='Scale of second criterion')
    ...
    strap_idx = args.strap_idx[0]
    train_size = args.train_size[0]
    layer_size = args.layer_size[0]
    learning_rate = args.learning_rate[0]
    batch_size = args.batch_size[0]
    crit_scale = args.crit_scale[0]
    crit_2_scale = args.crit_2_scale[0]
    ...
    data_dir = 'src/data'
    version_name = f'{version}_trainsize_{train_size}_layersize_{layer_size}_learningrate_{learning_rate}_batchsize_{batch_size}_crit1_{crit_scale}_crit2_{crit_2_scale}_bootstrap_{strap_idx}'
    ...
    train_idx, test_idx = split_data(full_indices, split_size=test_size, shuffle=True, rng=rng)
    train_idx = train_idx[:int(train_size*len(train_idx))]
    train_idx = bootstrap(train_idx, bootstrap_idx=strap_idx, shuffle=True, bootstrap_seed=seed)
    print('Train count: ', len(train_idx), 'Test count: ', len(test_idx))

    scaler = make_toy_fcn_standardscaler(data_dir, rotation=0, indices=train_idx,
                                         pixelization=pixelization)

    model = FCN_aug(layer_size=layer_size, pixelization=pixelization)
    train_dataset = toy_fcn_aug_set_dataset(data_dir, indices=train_idx, scaler=scaler,
                                        pixelization=pixelization)
    ...
    train_kwargs = {'learning_rate': learning_rate*world_size, 
                    ...
                    'criterion': criterion, 'criterion_2': criterion_2,
                    'crit_scale': crit_scale, 'crit_2_scale': crit_2_scale,
    ...
    test_model = FCN_aug(layer_size=layer_size, pixelization=pixelization)
    test_dataset = toy_fcn_dataset(data_dir, rotation=0, indices=test_idx, scaler=scaler,
                                   pixelization=pixelization)
